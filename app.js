...
function demonstrate() {
  ...
  // FIX 1: Invalid string handling — check membership before any pumping logic
  if (state.mode !== "cfl" && state.language !== "custom") {
    const langForCheck = LANGUAGE_PRESETS[state.language];
    if (langForCheck && !langForCheck.isInLanguage(rawStr)) {
      showInputError("Invalid string for the selected language. Please enter a valid string.");
      return;
    }
  } else if (state.mode === "cfl") {
    const cflLangForCheck = CFL_PRESETS[state.cflLanguage];
    if (cflLangForCheck && !cflLangForCheck.isInLanguage(rawStr)) {
      showInputError("Invalid string for the selected language. Please enter a valid string.");
      return;
    }
  }
...
function renderPumpRows(x, y, z, highlightI, langKey) {
  // FIX 2: Only auto-render rows 0..highlightI; higher rows shown only on explicit tab click
  els.pumpDisplay.innerHTML = "";
  const maxI = highlightI;
